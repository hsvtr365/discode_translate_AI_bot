...
특정 채널에서 지정된 사용자들의 대화를 실시간으로 한국어/영어 번역해주는 디스코드 봇입니다. 

## 기능 및 특징
- **계정별 번역 방향 지정**:
  - `USER_EN_ID`: 영어 원문으로 간주하여 한국어로 번역
  - `USER_KO_ID`: 한국어 원문으로 간주하여 영어로 번역
  - `USER_AUTO_ID` 또는 `USER_AUTO_IDS`: 영어/한국어를 자동 판별해 반대 방향으로 번역
- **자동 계정 판별 규칙**:
  - 한글 문자가 영문 문자보다 많거나 같으면 한국어로 판단하여 영어로 번역
  - 영문 문자가 더 많으면 영어로 판단하여 한국어로 번역
  - 한글/영문이 모두 없으면 응답하지 않고 건너뜀
...
- `USER_AUTO_ID`: 자동 반대 번역 계정 1개를 넣을 때 사용하는 디스코드 숫자 ID
- `USER_AUTO_IDS`: 자동 반대 번역 계정을 여러 개 넣을 때 사용하는 디스코드 숫자 ID 목록 (쉼표 구분)
...
3. **테스트 시나리오 3: 자동 반대 번역 확인**
   - 대상: `USER_AUTO_ID`
   - 디스코드 동작 1: 허용된 채널에 `USER_AUTO_ID` 계정으로 `Can we move the deployment to Friday?` 전송.
   - 예상 결과 1: 봇이 `🇰🇷 배포를 금요일로 미룰 수 있을까요?`처럼 한국어 번역 전송.
   - 디스코드 동작 2: 같은 계정으로 `오늘 안에 검토 끝내드릴게요.` 전송.
   - 예상 결과 2: 봇이 `🇺🇸 I'll finish the review by today.`처럼 영어 번역 전송.

4. **테스트 시나리오 4: 기능 비활성화 및 무시 규칙 확인**
   - 디스코드 동작 1: 허용되지 않은 다른 계정으로 `Hello?` 입력. -> 응답 없음 (무시)
   ...
   - 디스코드 동작 4: `USER_AUTO_ID`가 `12345` 또는 이모지처럼 한글/영문이 없는 메시지를 입력. -> 언어 판별 불가로 응답 없음.
